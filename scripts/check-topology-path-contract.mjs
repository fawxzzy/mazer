...
export const collectProtectedPathViolationsForTopologyPathContract = (changedFiles, decisionRegistry) => {
  ...
  if (!Array.isArray(assignments)) {
    return [violation(
      ...
      'Wave 2B requires the canonical dependency-ordered integrator ownership assignments.'
    )];
  }
  ...
  for (const file of changedFiles) {
    ...
    if (assignedWave !== undefined && assignedWave !== '2B') {
      violations.push(violation(
        'integrator-wave-ownership-mismatch',
        normalized,
        `"${normalized}" belongs to Wave ${assignedWave} and must not be modified by Wave 2B.`
      ));
    }
  }
  return violations;
