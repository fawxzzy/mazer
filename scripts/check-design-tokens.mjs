...
export const collectProtectedPathViolationsForTokens = (changedFiles, decisionRegistry) => {
  ...
  if (!Array.isArray(assignments)) {
    return [violation(
      ...
      'Wave 1B requires the canonical dependency-ordered integrator ownership assignments.'
    )];
  }
  ...
  for (const file of changedFiles) {
    ...
    if (assignedWave !== undefined && assignedWave !== '1B') {
      violations.push(violation(
        'integrator-wave-ownership-mismatch',
        normalized,
        `"${normalized}" belongs to Wave ${assignedWave} and must not be modified by Wave 1B.`
      ));
    }
  }
  return violations;
