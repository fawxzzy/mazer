...
import { TELEPORT_REQUIRED_TESTS } from './teleport-required-tests.mjs';
...
const missing = TELEPORT_REQUIRED_TESTS.filter(
  (relativePath) => !existsSync(resolve(REPO_ROOT, relativePath))
);
if (missing.length > 0) {
  console.error(`FATAL: Teleport System v1 required test file(s) missing -- cannot verify coverage: ${missing.join(', ')}`);
  process.exitCode = 1;
} else {
  runVitest([...TELEPORT_REQUIRED_TESTS, '--maxWorkers=1']);
}
