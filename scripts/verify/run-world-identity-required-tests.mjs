...
import { WORLD_IDENTITY_REQUIRED_TESTS } from './world-identity-required-tests.mjs';
...
const missing = WORLD_IDENTITY_REQUIRED_TESTS.filter(
  (relativePath) => !existsSync(resolve(REPO_ROOT, relativePath))
);
if (missing.length > 0) {
  console.error(`FATAL: World Identity required test file(s) missing -- cannot verify coverage: ${missing.join(', ')}`);
  process.exitCode = 1;
} else {
  runVitest([...WORLD_IDENTITY_REQUIRED_TESTS, '--maxWorkers=1']);
}
