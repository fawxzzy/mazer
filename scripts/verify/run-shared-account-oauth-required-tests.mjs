...
import { SHARED_ACCOUNT_OAUTH_REQUIRED_TESTS } from './shared-account-oauth-required-tests.mjs';
...
const missing = SHARED_ACCOUNT_OAUTH_REQUIRED_TESTS.filter(
  (relativePath) => !existsSync(resolve(REPO_ROOT, relativePath))
);
if (missing.length > 0) {
  console.error(`FATAL: Wave 5C required test file(s) missing -- cannot verify coverage: ${missing.join(', ')}`);
  process.exitCode = 1;
} else {
  runVitest([...SHARED_ACCOUNT_OAUTH_REQUIRED_TESTS, '--maxWorkers=1']);
}
