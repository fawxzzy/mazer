...
import { MEASURED_BASELINE_REQUIRED_TESTS } from './measured-baseline-required-tests.mjs';
...
const missing = MEASURED_BASELINE_REQUIRED_TESTS.filter(
  (relativePath) => !existsSync(resolve(REPO_ROOT, relativePath))
);
if (missing.length > 0) {
  console.error(`FATAL: Wave 0C required test file(s) missing -- cannot verify coverage: ${missing.join(', ')}`);
  process.exitCode = 1;
} else {
  runVitest([...MEASURED_BASELINE_REQUIRED_TESTS, '--maxWorkers=1']);
}
