...
| live runtime diagnostics bridge | `src/scenes/menuRuntimeDiagnostics.ts`, `src/scenes/MenuScene.ts`, `src/legacy-runtime/mazeCycleTelemetry.ts` | `tests/scenes/menu-runtime-diagnostics.test.ts`, `tests/reset/legacy-reset.test.ts`, `tests/reset/legacy-cycle-telemetry.test.ts`, `tests/visual/edge-live-check.test.ts`, localhost; diagnostics are data-only and do not draw a visible proof/debug panel over the game, `generation.maze` exposes source/build family plus compact maze quality stats, and `cycleTelemetry.learning` exposes local-only completion-pattern summaries for maintained-browser proof |
...
| menu/play dynamic overlay sizing | `src/legacy-runtime/legacyMenuRender.ts`, `src/scenes/MenuScene.ts` | `tests/scenes/menu-render-frame.test.ts`; trail stroke width, start/goal marker inset, and player halo/core metrics clamp for ultra-narrow mobile tiles while preserving desktop weight |
...
- if the change is "how shortcut branches or alternate start-goal routes are carved?", start in `src/legacy-runtime/legacyMaze.ts` for active reset-lane checkpoint/backtrack, `_WallArray` shortcut selection, route-quality measurement, and weak-route reinforcement, then inspect `src/domain/maze/core.ts` at `braidMaze()`, `resolveBraidShortcutProfile()`, `measureRouteReconnectionSpan()`, and `applyRouteAwareBypassPass()`, then inspect `src/domain/maze/generator.ts` at the raster legacy bridge pass
