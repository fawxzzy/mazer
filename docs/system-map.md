...
- if the change is "why does the menu board reveal by rows after generation?", start in `MenuScene.armLegacyMenuStaticDrawStage()` and the stage-6 execution plan
...
| menu trench width / connection-aware core continuity / core-vs-grid read | `src/legacy-runtime/legacyMenuRender.ts` + `src/scenes/MenuScene.ts` |
...
- `src/legacy-runtime/legacyMenuRender.ts`
  - owns menu-only trench edge/core frame math, including connection-aware light-core continuity
