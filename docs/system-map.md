...
| active-play keyboard and mobile pointer movement | `src/legacy-runtime/legacyPlayStep.ts`, `src/scenes/MenuScene.ts` | `tests/reset/legacy-play-step.test.ts`, localhost mobile play proof; pointer/touch swipes and short taps resolve into the same one-step vector and axis-gated collision path as keyboard input |
