...
- active-play movement, collision, mobile pointer/touch input, and reset-return ownership are closer after the simultaneous-key buffer, keyboard default-consumption guard, browser focus-loss cleanup, board-bounded pointer-vector adapter, axis-gated collision, single reset-request, and pause-reset trail-preservation ports
...
- exact active-play feel beyond the keyboard/pointer/focus-loss movement adapters, pause-reset trail preservation, active dynamic corridor overlays, extracted source-exact timer/goal-arrow contract, tighter compact overlay, minimal HUD widget chrome, data-only diagnostics proof, and full proof bounds
