...
- shortcut topology is stronger than the earlier browser-native random dead-end braider and now proves multiple route-bypass bands plus legacy opposite-corridor bridges in both the domain raster path and active reset-lane play snapshot path; the active reset-lane shortcut pass now consumes a duplicate-preserving `_WallArray` collected during the checkpoint path-builder's `CreatePath` equivalent, reports requested/attempted/created shortcut stats, and now publishes route-quality stats that prove shortcut-enabled play snapshots retain alternate start-goal route capacity, but exact legacy randomness remains a partial port
