...
- legacy front-door `Exit` now routes through an explicit browser-safe quit equivalence contract instead of a temporary explanation overlay, ultra-narrow side-panel menu widths now use a vertical front-door button stack plus a viewport-fitted board instead of the desktop row, and the dead `message` overlay family has been removed from the active reset lane
