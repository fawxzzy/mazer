...
- menu stage-6 row reveal is intentionally watchable: the static board now reveals one row per cadence tick, menu dynamic trail/player/goal overlays are clamped to already revealed rows, and menu demo movement waits for build completion plus a short settle so the main-menu maze assembles visibly instead of flashing in fully built
