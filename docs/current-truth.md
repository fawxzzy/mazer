...
- menu stage-6 reveal is intentionally watchable: the board glass/walls establish first, then real generated walkable path tiles reveal in solution-spine order with nearby carved branches following, menu dynamic trail/player/goal overlays are clamped to already revealed tiles, and menu demo movement waits for tile-build completion plus a short settle so the main-menu maze assembles visibly instead of flashing in fully built
