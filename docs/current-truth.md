...
- mobile maze crispness is handled at the geometry layer: canvas CSS no longer forces whole-surface pixelated/crisp-edge sampling, and board wall/path/cue rectangles snap to integer canvas-pixel boundaries so the maze stays sharper on phone-sized viewports without making text and gothic-cyber chrome chunky.
...
- front-door `Exit` has been intentionally removed from the active web runtime, including the old browser-safe quit equivalence module; the menu now exposes only `Start` and `Options`, ultra-narrow side-panel menu widths use a two-button vertical stack, viewport-fitted board, viewport-capped title scale, and a source-aware tighter title scale for dense `menu-generated` boards, and the dead `message` overlay family remains removed from the active reset lane
...
- front-door button layout now uses a centered two-button `Start` / `Options` pair in desktop and portrait layouts, with a two-button stack in ultra-narrow panes, removing the old unused quit button and its empty layout pressure
- legacy menu backdrop field now routes through an explicit backdrop owner contract in `src/legacy-runtime/legacyMenuBackdrop.ts`; the current mobile-clean direction uses a centered radial-warp pixel starfield, thin edge-weighted glass/rune shards, and small drift runes instead of the prior vertical falling-star/floating circular light/orb treatment, keeping the gothic cyber motif sharper without claiming final screenshot-grade closure
...
- front-door title pieces now carry a bounded animated sigil treatment on top of the path-piece wordmark: top/bottom rails, small crown glyphs, and a green/cyan prism sweep refresh only the title layer at a `90ms` cadence with a `2200ms` sweep diagnostic, keeping the logo more unique without adding broad blur or per-frame board redraws.
- mobile visual crispness now comes from integer-snapped maze wall/path/cue rectangles instead of forcing whole-canvas pixelated CSS scaling; visual diagnostics publish CSS canvas size, backing pixel size, DPR, and render-resolution ratio for proof.
