...
- active-play pointer/touch input now maps mobile swipes and short taps into the same one-step vector contract used by keyboard movement before passing through the legacy axis-gated collision resolver
...
- active-play movement, collision, mobile pointer/touch input, and reset-return ownership are closer after the simultaneous-key buffer, pointer-vector adapter, axis-gated collision, and single reset-request ports
