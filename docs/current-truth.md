...
- active-play board material is closer after replacing square debug-cell path fills with connected corridors, moving active dynamic overlays into a corridor/inset lane, centering the player marker with a high-contrast halo/core glyph, and removing the static path relief shadow so the generated maze reads as a cleaner 2D surface; final old-game play-board styling is not yet screenshot-grade
