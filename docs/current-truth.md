...
- maze generation process/stage ownership is mapped, and browser shortcut topology now uses family-aware route-affecting bypasses instead of random dead-end wall punches, but the browser topology builder is still not a line-for-line Unreal `CreateGrid` / `MapPath` / `CreatePath` / `CreateShortCuts` port
...
- shortcut topology is stronger than the earlier browser-native random dead-end braider, but it remains an improved browser equivalence rather than a literal Unreal `CreateShortCuts` source port
