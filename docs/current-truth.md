...
- active-play movement, collision, mobile pointer/touch input, and reset-return ownership are closer after the simultaneous-key buffer, keyboard default-consumption guard, browser focus-loss cleanup, board-bounded pointer-vector adapter, camera-follow layer alignment, axis-gated collision, single reset-request, pause-reset trail-preservation ports, and data-only input-buffer diagnostics for maintained-browser feel review
