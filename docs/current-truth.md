...
- active-play directional intent now resolves through one wrap-aware `LegacyDirectionalIntentResolver` before the world-turn movement phase: taps remain one accepted step, held keyboard/touch/stick intent continues at the configured cadence, and a newer direction replaces the single queued turn and is consumed at its first legal opening. When Smart Steering is enabled and the held direction meets a wall, assistance may move exactly one perpendicular tile when that side immediately restores the held lane; the held heading remains active and resumes on the next cadence step. A stable orthogonal order resolves two qualifying one-tile sides instead of freezing input, while disabled assistance, ordinary corners, chained/two-tile detours, dead ends, and mismatched queued turns still stop. Release, focus loss, pause, reset, generation, lifecycle transitions, and Smart Steering changes clear or synchronize intent. Runtime diagnostics publish only bounded active/queued directions, requested candidates, assisted lane-shift count/one-tile limit, and the last decision, never a future route.
...
- The maintained `390x844` authenticated Options/Play/Pause capture now checks a single normalized viewport owner across those surfaces. Menu and play share one two-row run-status component for time/rank plus score and maze level; the portrait play instance occupies the measured lane left of Pause rather than shrinking below its text width. Options and Pause share a scroll-body/pinned-footer composition, reserve the guide from the scroll rail, suppress empty clipped guide fragments, and separate the Move Speed label and slider lanes. The page, install metadata, and Phaser shell use one near-black safe-area color so browser chrome or PWA insets cannot expose purple bands.
