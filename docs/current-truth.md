...
- maze generation process/stage ownership is mapped, and browser shortcut topology now uses family-aware route-affecting bypasses with separated route-reconnection proof instead of random dead-end wall punches, but the browser topology builder is still not a line-for-line Unreal `CreateGrid` / `MapPath` / `CreatePath` / `CreateShortCuts` port
...
- browser shortcut braiding now scores wall openings by useful loop distance, applies family-specific shortcut profiles, disables weak fallback shortcuts for sparse mazes, and adds a bounded route-aware braided bypass pass after endpoint selection that requires separated route reconnection before opening the wall, so braided mazes can expose more than one start-goal route without collapsing dense/sparse identity
...
- shortcut topology is stronger than the earlier browser-native random dead-end braider and now proves multiple route-bypass bands for the covered braided seed, but it remains an improved browser equivalence rather than a literal Unreal `CreateShortCuts` source port
...
- the browser builder still resolves maze topology before stage-6 row reveal, so exact topology internals remain a future gameplay/topology concern rather than an unstated lifecycle claim; the visible row reveal is now paced, but it is not a line-for-line Unreal generator port
