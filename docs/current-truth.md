...
- legacy menu backdrop field now routes through an explicit backdrop owner contract in `src/legacy-runtime/legacyMenuBackdrop.ts`; the current mobile-clean direction uses pixel stars, angular glass shards, and small drift runes instead of the prior floating circular light/orb treatment, keeping the gothic cyber motif sharper without claiming final screenshot-grade closure
