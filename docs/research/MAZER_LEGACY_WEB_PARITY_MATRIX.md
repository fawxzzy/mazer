...
| HUD timer and end arrow | `Source/Mazer/Private/UI/GameHud.cpp`, `Source/Mazer/Private/Player/MazerPlayer.cpp` | `src/legacy-runtime/legacyPlayHud.ts`, `src/scenes/MenuScene.ts` | `partial` | HUD timer formatting now matches the restored player source's bare `M:SS` text and `% 10` minute wrap, goal-arrow angle math publishes radians and degrees, bounds are tighter, and scene wiring plus visual diagnostics readback are repo-owned and tested for active play. Final legacy HUD material, exact old-widget placement, and diagnostics-free visual styling remain open | Finish screenshot-grade HUD visual polish and active-play HUD edge-case review |
...
But final HUD visual styling and screenshot-grade visual composition still differ materially from the restored legacy project. Active-play maze rendering is less debug-grid-like after the connected-corridor pass, but final play-board material parity remains open.
