...
| HUD timer and end arrow | `Source/Mazer/Private/UI/GameHud.cpp`, `Source/Mazer/Private/Player/MazerPlayer.cpp` | `src/legacy-runtime/legacyPlayHud.ts`, `src/scenes/MenuScene.ts` | `partial` | HUD timer formatting, goal-arrow angle math, bounds, scene wiring, and visual diagnostics readback are now repo-owned and tested for active play. Final legacy HUD material, exact old-widget placement, and diagnostics-free visual styling remain open | Finish screenshot-grade HUD visual polish and active-play HUD edge-case review |
