...
| --- | --- | --- | --- | --- | --- |
| Maze generation lifecycle | `Source/Mazer/MazerGameModeBase.cpp`, `Source/Mazer/Private/MazerGameState.cpp` | `src/legacy-runtime/legacyGenerationLifecycle.ts`, `src/legacy-runtime/legacyMaze.ts`, `src/scenes/MenuScene.ts` | `partial` | Current web build path is structured and diagnosable, but still not a literal process-`0/3/4/5/6/7/8` legacy pipeline | Port the explicit staged lifecycle into the reset-lane runtime contract |
| Menu demo AI walker | `Source/Mazer/Private/Player/MazerPlayer.cpp` | `src/legacy-runtime/legacyMenuDemoLifecycle.ts`, `src/domain/ai/demoWalker.ts`, `src/scenes/MenuScene.ts` | `partial` | Demo motion is now modular and deterministic, but legacy backtracking, cadence, and reset semantics are still not exact | Re-port legacy demo walker decisions, backtrack policy, and reset semantics exactly |
...
| Options overlay | `Source/Mazer/Private/UI/PauseMenuWidget.cpp` | `src/legacy-runtime/legacyOptionFields.ts`, `src/scenes/MenuScene.ts` | `partial` | The options surface exists again, but field-level semantics and exact routing still need tighter parity proof | Finish field-by-field options parity and rebuild triggers |
| Features overlay | `Source/Mazer/Private/UI/FeaturesWidget.cpp` | `src/scenes/MenuScene.ts` | `partial` | Features exists as a first-class overlay, but exact toggle behavior and interaction details still need legacy verification | Tighten features overlay responsibilities against restored UI truth |
| Game modes overlay | `Source/Mazer/Private/UI/GameModesWidget.cpp` | `src/scenes/MenuScene.ts` | `partial` | Game modes exists as a first-class overlay, but the dark-mode and related responsibilities still need exactness verification | Tighten game-mode overlay responsibilities against restored UI truth |
| In-game pause menu | `Source/Mazer/Private/UI/GamePauseMenu.cpp` | `src/scenes/MenuScene.ts` | `partial` | Pause now exists inside the legacy overlay family, but the exact command routing and field responsibilities still need parity cleanup | Match pause overlay command routing and fields more exactly |
