...
| --- | --- | --- | --- | --- | --- |
| Maze generation lifecycle | `Source/Mazer/MazerGameModeBase.cpp`, `Source/Mazer/Private/MazerGameState.cpp` | `src/legacy-runtime/legacyGenerationLifecycle.ts`, `src/legacy-runtime/legacyMaze.ts`, `src/scenes/MenuScene.ts` | `partial` | Current web build path now queues named reset/generation requests instead of collapsing them into immediate rebuilds, but it is still not a literal process-`0/3/4/5/6/7/8` legacy pipeline | Port the remaining staged lifecycle into the reset-lane runtime contract |
