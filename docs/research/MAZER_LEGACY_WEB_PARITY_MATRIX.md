...
| Options overlay | `Source/Mazer/Private/UI/PauseMenuWidget.cpp` | `src/legacy-runtime/legacyOptionFields.ts`, `src/legacy-runtime/legacyOverlayFieldCommit.ts`, `src/scenes/MenuScene.ts` | `close` | The options surface now carries explicit deferred reload-on-back vs camera-flag field semantics, but final nested-overlay verification is still open | Keep the current field-commit contract while verifying the remaining nested-overlay behavior |
...
| In-game pause menu | `Source/Mazer/Private/UI/GamePauseMenu.cpp` | `src/legacy-runtime/legacyOverlayFieldCommit.ts`, `src/legacy-runtime/legacyPauseLifecycle.ts`, `src/scenes/MenuScene.ts` | `close` | Pause now exists inside the legacy overlay family, its command routing is explicit, and its camera-scale field responsibility is explicit, but final nested-overlay verification is still open | Keep the current pause contract while verifying the remaining nested-overlay behavior |
