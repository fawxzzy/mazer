...
| --- | --- | --- | --- | --- | --- |
| Maze generation lifecycle | `Source/Mazer/MazerGameModeBase.cpp`, `Source/Mazer/Private/MazerGameState.cpp` | `src/legacy-runtime/legacyGenerationLifecycle.ts`, `src/legacy-runtime/legacyPlayLifecycle.ts`, `src/legacy-runtime/legacyMaze.ts`, `src/scenes/MenuScene.ts` | `partial` | Current web build path now queues named reset/generation requests and carries explicit stage-7/process-8 branch contracts, but it is still not a literal process-`0/3/4/5/6/7/8` legacy pipeline | Port the remaining staged lifecycle into the reset-lane runtime contract |
