...
| Visual composition | legacy screenshots `menu-01..04` | `src/legacy-runtime/legacyMenuSnapshot.ts`, `src/legacy-runtime/legacyMenuLayout.ts`, `src/legacy-runtime/legacyMenuTitle.ts`, `src/legacy-runtime/legacyMenuButtonChrome.ts`, `src/legacy-runtime/legacyMenuBackdrop.ts`, `src/legacy-runtime/legacyMenuRender.ts`, `src/scenes/MenuScene.ts` | `partial` | The desktop board now occupies the frame more like the restored legacy screenshots, the title lockup reads closer to the legacy wordmark treatment, the front-door support chrome is more compact, the backdrop field now has an explicit owner plus a denser cloudy/star treatment, and the board material reads darker and less evenly tiled, but final backdrop and board-material closure is still open | Finish screenshot-grade board placement, title treatment, backdrop, and button layout |
