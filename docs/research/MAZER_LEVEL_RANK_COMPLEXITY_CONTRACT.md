...
- Replay reads the current player and AI server cursors before it submits queued work. A provable telemetry chain is prepended in exact sequential order to repair a stale or gapped outbox, duplicate run UUIDs are deduplicated, and retry remains idempotent. If local telemetry cannot prove every missing completion, the higher local state and pending receipt remain intact without a speculative server write.
