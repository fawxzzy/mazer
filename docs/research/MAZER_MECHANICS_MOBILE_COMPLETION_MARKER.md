...
| Mobile input and active-play usability | `15` | `14` | live touch movement plus partial touch-control proof | `src/scenes/MenuScene.ts`, `src/input-human/touch.ts`, `src/legacy-runtime/legacyPlayStep.ts`, `src/legacy-runtime/legacyMenuLayout.ts` | close same-button pause resume under the pause overlay, add visible mobile control affordances, and continue mobile board/player readability checks after layout tuning |
