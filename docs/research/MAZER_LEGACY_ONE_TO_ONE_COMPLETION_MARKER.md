...
| Demo route, backtracking, and pacing | `12` | `9` | partial | `src/legacy-runtime/legacyMenuDemoLifecycle.ts` -> `src/domain/ai/demoWalker.ts` -> `src/scenes/MenuScene.ts` | `tests/ai/demo-walker.test.ts`, `tests/reset/legacy-menu-demo-lifecycle.test.ts`, localhost | recovery cues, AI-only reset replay, goal-reset timing, `AiTilePathCheck` admission, and source-shaped neighbor-scan / potential-tile / path-stack backtracking route planning are covered; exact Unreal material color-revert timing, blueprint AI cadence, and full visited-flag side effects remain open |
...
## Why the marker was corrected from 97% to 70%, then ratcheted to 90%
