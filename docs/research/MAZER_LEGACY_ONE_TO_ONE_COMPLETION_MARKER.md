...
| Overlay family and field responsibilities | `14` | `14` | close | `src/legacy-runtime/legacyOptionFields.ts` -> `src/legacy-runtime/legacyOverlayFieldCommit.ts` -> `src/legacy-runtime/legacyOverlayToggleFields.ts` -> `src/legacy-runtime/legacyPauseLifecycle.ts` -> `src/scenes/MenuScene.ts` | `tests/reset/legacy-option-fields.test.ts`, `tests/reset/legacy-overlay-field-commit.test.ts`, `tests/reset/legacy-overlay-toggle-fields.test.ts`, `tests/reset/legacy-pause-lifecycle.test.ts`, `tests/reset/legacy-reset.test.ts`, localhost | field responsibilities are now explicit for options, features, game modes, and pause commands, but final nested-overlay verification from menu/pause parents still remains open |
...
- final nested-overlay verification still needs proof even though overlay field and command responsibilities are now explicit
