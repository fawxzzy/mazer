...
## Why the marker was corrected from 97% to 70%, then ratcheted to 72%
...
- shortcut topology is no longer the earlier random dead-end braider: it now uses family-aware loop scoring, sparse fallback protection, a bounded route-aware braided bypass pass that rejects candidates without separated canonical-route reconnection, a raster-level legacy opposite-corridor bridge pass, and an active reset-lane `createLegacyMaze()` shortcut bridge pass using the explicit legacy budget; this improves route alternatives and ports the core `CreateShortCuts` condition onto the active runtime owner, but does not close the full line-for-line generator port gap
