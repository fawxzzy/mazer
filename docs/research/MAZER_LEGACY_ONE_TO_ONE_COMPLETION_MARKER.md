...
## Why the marker was corrected from 97% to 70%, then ratcheted to 73%
...
- shortcut topology is no longer the earlier random dead-end braider: it now uses family-aware loop scoring, sparse fallback protection, a bounded route-aware braided bypass pass that rejects candidates without separated canonical-route reconnection, a raster-level legacy opposite-corridor bridge pass, and an active reset-lane `createLegacyMaze()` shortcut bridge pass using the explicit legacy budget and a duplicate-preserving `_WallArray`-style random-removal loop; this improves route alternatives and ports the core `CreateShortCuts` condition plus selection lifecycle shape onto the active runtime owner, but does not close the full line-for-line generator port gap
...
- the active runtime shortcut bridge packet earns one point because it moves the restored opposite-corridor shortcut rule into `src/legacy-runtime/legacyMaze.ts`, which is the active reset-lane maze owner used by `MenuScene`, and wires the explicit legacy shortcut budget into play-mode generation; it does not earn more because the full Unreal staged wall-array lifecycle and checkpoint path builder remain open
- the active runtime wall-array shortcut packet earns one point because active reset-lane shortcut selection now builds a `_WallArray`-style duplicate-preserving candidate list from path-neighbor walls, randomly removes one entry per attempt, revalidates stale candidates before opening, and reports requested/attempted/created shortcut stats; it does not earn more because the full Unreal checkpoint path builder and exact non-deterministic RNG source remain open
