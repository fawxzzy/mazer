...
| Overlay family and field responsibilities | `14` | `12` | partial | `src/legacy-runtime/legacyOptionFields.ts` -> `src/legacy-runtime/legacyPauseLifecycle.ts` -> `src/scenes/MenuScene.ts` | `tests/reset/legacy-option-fields.test.ts`, `tests/reset/legacy-pause-lifecycle.test.ts`, `tests/reset/legacy-reset.test.ts`, localhost | options/pause rebuild timing now defers to overlay close like legacy truth, and pause command routing is now explicit, but features/game-modes/pause fields still need broader field-by-field exactness |
...
- some overlay and front-door responsibilities still need exactness passes beyond the deferred options/pause rebuild contract and the explicit pause command lifecycle
