...
describe('resolveLegacyMenuPathRenderFrame', () => {
  ...
  test('keeps account login/logout inside the shared player-facing overlay system', () => {
    ...
    expect(menuSceneSource).toContain('username: resolveLegacyAuthUsernameDraft(snapshot, this.authForm.username, previousUserId)');
    expect(menuSceneSource).toContain('usernameDraftEmpty: this.authForm.username.length === 0');
