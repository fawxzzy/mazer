...
describe('legacy reset lane', () => {
  ...
  test('routes generation and reset through explicit queued request contracts', () => {
    ...
    expect(menuSceneSource).toContain('const rowTicks = Math.ceil(this.maze.size / batchSize);');
    expect(menuSceneSource).toContain('if (this.menuStaticDrawRowsVisible !== null) {');
