...
describe('directional intent architecture contract', () => {
  test('keeps input, graph, turn, rendering, and proof ownership explicit', () => {
    ...
    expect(contract).toContain('the resolver may move one tile on the perpendicular axis only when exactly one side tile is legal');
    ...
    expect(intentSource).toContain("return this.createStopStep('stopped-at-ambiguous-lane-shift');");
