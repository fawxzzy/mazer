...
describe('directional intent architecture contract', () => {
  test('keeps input, graph, turn, rendering, and proof ownership explicit', () => {
    ...
    expect(contract).toContain('`legacy-directional-intent-v1`');
    expect(contract).toContain('exactly one latest-wins queued direction');
    expect(contract).toContain('Automatic assistance stops at a dead end, a genuine two-branch ambiguity, or after four assisted turns.');
    ...
    expect(intentSource).toContain('export const LEGACY_DIRECTIONAL_INTENT_ASSISTED_TURN_LIMIT = 4;');
    expect(intentSource).toContain("return this.createStopStep('stopped-at-intersection');");
